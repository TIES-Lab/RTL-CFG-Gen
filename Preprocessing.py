...
def begin_end(file_path: str):
    with open(file_path, 'r') as fl:
        full_txt_lines = fl.readlines()
        fl.close()

    begin_pattern = re.compile(r"[.]+\s+begin\s+[.]+\n?")
    end_pattern = re.compile(r"[.]+\s+end\s+[.]+\n?")
    case_pattern = re.compile(r"[.]+\s+case\s*[.]+\n?")

    for iter_line in full_txt_lines:
        if re.search(begin_pattern, iter_line) is not None:
            re.sub(" begin ", " begin\n")
        elif re.search(end_pattern, iter_line) is not None:
            re.sub(" end ", "\nend\n")
        elif re.search(case_pattern, iter_line) is not None:
            re.sub(" case", "\ncase")

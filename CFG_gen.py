...
def always_blck_CFG(lines: list[str], index, node: str, Nodeinfo: list[str]):
    ...
    cond_onezero_pattern = re.compile(
        r"\s*([^=;\(\)<>]+)\s*(<?=)\s*([^=;\(\)<>]+)\s*(==|!=|<|<=|>|>=)\s*([^=;\(\)<>]+);")
    ...
    while iter_no < len(always_blck):
        ...
        elif re.search(cond_onezero_pattern, iter_line) is not None:
            node, Nodeinfo = cond_onezero(iter_line, node_root, Nodeinfo)
    ...
    if any(x in ifelse_trig for x in line_lst):
        ...
        while iter_no < len(ifelse_blck):
            ...
            if 'else' in iter_line_lst and 'if' in iter_line_lst:
                ...
                node, Nodeinfo = cond_onezero(iter_line, node, Nodeinfo)

            elif re.search(assgmnt_pttrn, iter_line) is not None and "if" not in iter_line_lst:
                node, Nodeinfo = get_assgnment(iter_line, node, Nodeinfo)

            elif re.search(inline_cond_pattern, iter_line) is not None and "if" not in iter_line_lst:
...
def nestedcasestatement(lines: list[str], index4: int, node: str, Nodeinfo: list[str]):
    ...
    if len(case_trig_act) > 0:
        ...
        while iter_value < len(case_blck[1:]):  # end of the case block will be lines[index4 + iter_value + 1]
            ...
            elif re.search(cond_onezero_pattern, nextline) is not None:
                node, Nodeinfo = cond_onezero(nextline, node, Nodeinfo)
...
def cond_onezero(line: str, node: str, Nodeinfo: list[str]) -> list[str]:
    """

    Parameters
    ----------
    line: Current Line that is a in-line conditional assignment operation
    node: The Node that the current line belongs to, with its parent node information
    Nodeinfo: Complete information of the Node, in numerical format along with the assignment and conditional assignment

    Returns: updated node number and Nodeinfo
    -------

    """

    pttrn = re.compile(r"\s*(assign)?\s*([^=;\(\)<>]+)\s*(<?=)\s*([^=;\(\)<>]+)\s*(==|!=|<|<=|>|>=)\s*([^=;\(\)<>]+);")
    line_lst = [x.strip() for x in list(re.findall(pttrn, line)[0])]
    ...
    assignee_var = line_lst[1]
    assign_cond_true = " ".join(line_lst[3::])
    line_lst_false = line_lst
    line_lst_false[4] = change_val[line_lst[4]]
    assign_cond_false = " ".join(line_lst_false[3::])

    # Assignment versions
    assgnmnt_true = assignee_var + " <== 1"
    assgnmnt_false = assignee_var + " <== 0"

    # True Condition
    node_t = node + ", 0"
    Nodeinfo_append0 = node_t + " :: " + assign_cond_true + " :: " + "C"
    Nodeinfo.append(Nodeinfo_append0)
    Nodeinfo_append0 = node_t + " :: " + assgnmnt_true + " :: " + "A"
    Nodeinfo.append(Nodeinfo_append0)

    # False Condition
    node_f = node + ", 1"
    Nodeinfo_append1 = node_f + " :: " + assign_cond_false + " :: " + "C"
    Nodeinfo.append(Nodeinfo_append1)
    Nodeinfo_append1 = node_f + " :: " + assgnmnt_false + " :: " + "A"
    Nodeinfo.append(Nodeinfo_append1)
...
def gen_CFG(file_loc: str):
    ...
    while line_no < len(lines):
        ...
        if re.search(cond_onezero_pattern, line) is not None and not any(x in line_lst for x in skip_term_lst):
            node, Nodeinfo = cond_onezero(line, node, Nodeinfo)
            curr_node += 1
        elif re.search(inline_cond_pattern, line) is not None and not any(x in line_lst for x in skip_term_lst):
...
def main():
    ...
    file_lst = ["Bug Insertion/adc_ctrl_fsm_buggy.sv"]
